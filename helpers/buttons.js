 ...
 *  6. Support for images and videos in button messages.
 *
 * Usage (minimal):
...
/**
 * Validate media object for interactive messages
...
function validateMedia(media) {
  ...
  } else if (!['image', 'video'].includes(media.type)) {
  ...
  if (media.caption && typeof media.caption !== 'string') {
    warnings.push('media.caption should be a string');
  }
...
function validateSendButtonsPayload(data) {
  ...
  if (data.media) {
    const mediaValidation = validateMedia(data.media);
...
/**
 * Process media for interactive messages
 * @param {object} media Media object
 * @returns {object} Processed media message
 */
...
function convertToInteractiveMessage(content) {
  if (content.interactiveButtons && content.interactiveButtons.length > 0) {
    ...
    // Optional header - can be text or media
    if (content.title || content.subtitle || content.media) {
      if (content.media) {
        // Use media as header
        const mediaMessage = processMedia(content.media);
        if (mediaMessage) {
          interactiveMessage.header = mediaMessage;
        }
      } else {
        // Use text as header
    ...
    // Body text (only if no media header or if we want both)
 ...
 * @param {object} [data.media] Media object for image/video support.
 * @param {string} [data.media.type] Media type - 'image' or 'video'
 * @param {string} [data.media.url] Media URL
 * @param {string} [data.media.caption] Optional media caption
...
async function sendInteractiveButtonsBasic(sock, jid, data = {}, options = {}) {
  ...
  const { text = '', footer = '', title, subtitle, buttons = [], media } = data;
  // Strict payload validation for sendButtons format.
  const strict = validateSendButtonsPayload({ text, buttons, title, subtitle, footer, media });
  if (!strict.valid) {
